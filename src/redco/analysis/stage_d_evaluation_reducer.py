"""Transition reducer for the append-only Stage-D evaluation ledger."""
...
from redco.integrity import is_sha256_hex, require_sha256_hex
...
def reduce_evaluation_records(
    ...
    *,
    ...
    """Validate and reduce a complete evaluation ledger into its immutable snapshot."""
    expected_authorization_sha256 = require_sha256_hex(
        expected_authorization_sha256,
        "expected evaluation authorization_sha256",
    )
    records = _stable_records(records)
    record_sha256s = tuple(record_sha256s)
    genesis_event = _validate_records(
        records,
        record_sha256s,
        manifest=manifest,
    ...
    for record, record_digest in zip(records[1:], record_sha256s[1:], strict=True):
        ...
        if kind == "server_launch_reserved":
            ...
            arm = _arm(event["arm"])
            epoch_index = _latest_epoch_index(server_epochs, arm)
            ...
            previous_client = next(
                (item for item in reversed(client_epochs) if item.arm == arm),
                None,
            )
            ...
            arm = _arm(event["arm"])
            epoch_index = _latest_epoch_index(client_epochs, arm)
            ...
            client_epoch_state = next(
                (item for item in reversed(client_epochs) if item.arm == unit.arm),
                None,
            )
            if (
            ...
            if not is_sha256_hex(attempt_id) or attempt_id in task_by_id:
            ...
            if (
                ...
                or not is_sha256_hex(event["event_address_sha256"])
            ):
            ...
            if call.dispatch_receipt_sha256 is not None:
                raise ValueError("evaluation call transition requires reserved")
            _store_call(
                tasks,
                call_to_task,
                replace(
            ...
            if call.dispatch_receipt_sha256 is None or call.response_envelope_sha256 is not None:
                raise ValueError("evaluation call transition requires dispatched")
            _store_call(
                tasks,
                call_to_task,
                replace(
            ...
            if call.response_envelope_sha256 is None or call.outcome_sha256 is not None:
                raise ValueError("evaluation call transition requires responded")
            _store_call(
                tasks,
                call_to_task,
                replace(call, outcome_sha256=event["outcome_sha256"]),
            )
    ...
    return EvaluationLedgerSnapshot(
        authorization_sha256=genesis_event["authorization_sha256"],
        execution_manifest_sha256=genesis_event["execution_manifest_sha256"],
        evaluation_plan_sha256=genesis_event["evaluation_plan_sha256"],
        created_at_unix_ns=genesis_event["created_at_unix_ns"],
...
def _validate_records(
    ...
    *,
    ...
    for offset, (record, digest) in enumerate(zip(records, record_sha256s, strict=True)):
        if record.get("offset") != offset or not is_sha256_hex(digest):
        ...
        for name, value in event.items():
            if name in _SHA_FIELDS and value is not None and not is_sha256_hex(value):
    ...
    body = cast(dict[str, Any], genesis["event"])
    authorization_sha256 = body["authorization_sha256"]
    if not is_sha256_hex(authorization_sha256):
        raise ValueError("evaluation ledger authorization_sha256 is invalid")
    if authorization_sha256 != expected_authorization_sha256:
        raise ValueError("evaluation ledger authorization differs from authenticated input")
    if (
...
def _stable_records(records: Sequence[dict[str, Any]]) -> tuple[dict[str, Any], ...]:
    """Copy one exact JSON representation before validation and reduction."""

    return tuple(cast(dict[str, Any], _copy_exact_json(record)) for record in records)


def _copy_exact_json(value: object, *, field: str | None = None) -> object:
    if value is None or type(value) in {bool, int, str}:
        return value
    if type(value) is list:
        return [_copy_exact_json(item) for item in cast(list[object], value)]
    if type(value) is dict:
        owner = cast(dict[object, object], value)
        if any(type(key) is not str for key in owner):
            raise ValueError("evaluation ledger record representation differs")
        return {
            cast(str, key): _copy_exact_json(item, field=cast(str, key))
            for key, item in owner.items()
        }
    if field in _SHA_FIELDS:
        raise ValueError(f"evaluation ledger {field} is invalid")
    raise ValueError("evaluation ledger record representation differs")
...
def _latest_epoch_index(
    epochs: Sequence[EvaluationServerEpoch | EvaluationProcessEpoch],
    arm: ArmName,
) -> int | None:
    return next((index for index in reversed(range(len(epochs))) if epochs[index].arm == arm), None)
...
def _store_call(
    tasks: list[EvaluationTaskState],
    call_to_task: dict[str, int],
    updated: EvaluationCallState,
) -> None:
    task_index = call_to_task[updated.call_id]
    task = tasks[task_index]
    calls = tuple(updated if item.call_id == updated.call_id else item for item in task.calls)
    tasks[task_index] = replace(task, calls=calls)


__all__ = ["reduce_evaluation_records"]
