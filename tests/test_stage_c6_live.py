...
from redco.analysis.stage_c6_live import RUNS
...
def test_stage_c6_uses_fresh_seed_block_and_unchanged_decision(tmp_path: Path) -> None:
    ...
    result = verify_campaign(
        ...
        score_path,
        run_seeds=RUNS,
        analysis="stage-c6-credit-confusion-live-v1",
    )

    assert result["status"] == "passed"
    assert result["analysis"] == "stage-c6-credit-confusion-live-v1"
    assert set(RUNS["confusion_irrelevant"]) == {9901, 9902}
