...
config_root="${REDCO_CONFIG_ROOT:-configs/stage-c6/rendered-v2}"
run_root="${REDCO_RUN_ROOT:-runs/stage-c6/credit-confusion-live-v2}"
...
structural_config="$config_root/structural-broadcast-s9910.toml"
...
export REDCO_UV_ENVIRONMENT="${REDCO_UV_ENVIRONMENT:-/home/ubuntu/.cache/redco/stage-c6-v2-env}"
...
for replicate in 1 2 3; do
  ...
  CUDA_VISIBLE_DEVICES=0 "${uv_prime[@]}" python \
    ...
    --action-output "$replicate_root/action-scores.json" \
    --root-output "$replicate_root/root-scores.json" \
    >"$replicate_root/score.log" 2>&1
...
# vLLM is retained only to verify power in the actual rollout deployment.
signed_field() {
  "${uv_prime[@]}" python -c \
...
structural_output="$run_root/smoke/structural-broadcast-s9910"
run_supervised \
  "$structural_config" \
  "$structural_output" \
...
confusion_irrelevant|sliced|9911
confusion_irrelevant|sliced|9912
confusion_irrelevant|broadcast|9912
confusion_redundant|broadcast|9913
confusion_redundant|sliced|9913
confusion_lucky|sliced|9914
confusion_lucky|broadcast|9914
RUNS
...
confusion_irrelevant|broadcast|9911|36
confusion_irrelevant|sliced|9911|6
confusion_irrelevant|sliced|9912|6
confusion_irrelevant|broadcast|9912|36
confusion_redundant|broadcast|9913|36
confusion_redundant|sliced|9913|6
confusion_lucky|sliced|9914|6
confusion_lucky|broadcast|9914|36
ADAPTERS
...
"${uv_prime[@]}" python -m redco.analysis.stage_c6_v2_live \
  --run-root "$run_root" \
