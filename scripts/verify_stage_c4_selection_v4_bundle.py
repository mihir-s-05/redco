...
def verify(bundle_root: Path) -> dict:
    ...
    selection_root = stage_root / "warmstart-selection-v4"
    sft_root = stage_root / "warmstart-sft-v4"
    ...
    prereg_audit = _load_signed(
        bundle_root
        / "reports/stage-c4-warmstart-selection-v4-preregistration-audit-2026-07-29.json"
    )
    if not prereg_audit["passed"]:
        raise ValueError("frozen preregistration audit did not pass")
    ...
    if (
        ...
        or selection["evaluated_steps"] != CANDIDATE_STEPS
    ...
    if set(loss_by_step) != set(range(1, 33)) or any(nan_by_step.values()):
        raise ValueError("SFT metrics do not contain 32 finite loss steps")

    return sign_payload(
        {
            "schema_version": 1,
            "analysis": "stage-c4-warmstart-selection-v4-terminal-verification",
            ...
            "preregistration_audit_signature": prereg_audit[
