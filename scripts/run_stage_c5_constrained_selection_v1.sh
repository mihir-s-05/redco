...
sft_timeout="${REDCO_SFT_TIMEOUT_SECONDS:-3600}"
...
# The first live model operation validates the actual multi-choice request path,
# generated token path, behavior logprobs, and serialized trainer logprobs.
CUDA_VISIBLE_DEVICES=1 "${uv_prime[@]}" python scripts/run_signed_vllm_scorer.py \
  --output "$selection_root/base-root-scores.json" \
  --verified "$selection_root/base-root-scores.verified.json" \
...
"${uv_prime[@]}" python scripts/verify_stage_c5_constraint_smoke.py \
  --traces "$smoke_run/run_default/rollouts/step_1/train/all/traces.jsonl" \
  --batch "$smoke_run/run_default/rollouts/step_1/train_rollouts.bin" \
  --root-scores "$selection_root/base-root-scores.json" \
  --expected-context-traces 8 \
  --output "$selection_root/constrained-interface-smoke.json"
smoke_status="$(
  "${uv_prime[@]}" python -c \
    'import json,sys; print(json.load(open(sys.argv[1]))["status"])' \
    "$selection_root/constrained-interface-smoke.json"
)"
if test "$smoke_status" != "passed"; then
  touch "$selection_root/CONSTRAINED_INTERFACE_REJECTED"
  exit 31
fi

nvidia-smi \
  --query-gpu=timestamp,index,name,memory.used,memory.total,utilization.gpu,power.draw \
  --format=csv,noheader >"$selection_root/resource-before-sft.csv"

timeout --signal=TERM "$sft_timeout" \
...
"${uv_prime[@]}" python scripts/evaluate_stage_c5_candidate.py select \
  "${candidate_report_args[@]}" \
  --output "$selection_root/selection.json"

if test -z "$selected_step"; then
  cleanup_candidate_merged
  touch "$selection_root/INITIALIZATION_ENGINEERING_ENDED"
  nvidia-smi \
    --query-gpu=timestamp,index,name,memory.used,memory.total,utilization.gpu,power.draw \
