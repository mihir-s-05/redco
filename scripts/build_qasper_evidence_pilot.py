"""Build the compact QASPER evidence pilot from the pre-cleanup Git archive."""
...
from redco.experiments.qasper_evidence import PilotBudget, build_pilot_tasks
...
OUTPUT_PATH = Path("data/qasper-evidence-pilot-v1.json")
...
def main() -> None:
    ...
    if arguments.check:
        if not OUTPUT_PATH.is_file() or OUTPUT_PATH.read_bytes() != rendered:
            raise SystemExit("pilot dataset is missing or stale")
        return
