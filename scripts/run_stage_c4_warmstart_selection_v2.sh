...
config="configs/stage-c4/factorized-warmstart-sft-v2.toml"
...
base_merged="runs/stage-c4/base-merged-step23-v2"
lifecycle_root="runs/stage-c4/scorer-lifecycle-v2"
sft_run="runs/stage-c4/warmstart-sft-v2"
selection_root="runs/stage-c4/warmstart-selection-v2"
selected_root="runs/stage-c4/warmstart-selected-v2"
...
mkdir -p "$XDG_CONFIG_HOME" "$lifecycle_root" \
  "$selection_root/candidates" "$selection_root/work"
...
cleanup_candidate_merged() {
  local expected="$repo_root/runs/stage-c4/warmstart-selection-v2/work/merged-candidate"
...
# Crash-proof lifecycle gate: both scorers must return zero and have their
# durable signed output independently committed by the parent supervisor.
"${uv_prime[@]}" python scripts/run_signed_vllm_scorer.py \
  --output "$lifecycle_root/action-scores.json" \
  --verified "$lifecycle_root/action-scores.verified.json" \
  --expected-cases-sha256 "$action_cases_signed" \
  --expected-model "$base_merged" \
  -- \
  python scripts/score_stage_c_policies_vllm.py \
    --cases "$action_cases" \
    --model "$base_merged" \
    --model-name lifecycle_base \
    --tensor-parallel-size 1 \
    --gpu-memory-utilization 0.7 \
    --output "$lifecycle_root/action-scores.json" \
  >"$lifecycle_root/action-score.log" 2>&1

"${uv_prime[@]}" python scripts/run_signed_vllm_scorer.py \
  --output "$lifecycle_root/root-scores.json" \
  --verified "$lifecycle_root/root-scores.verified.json" \
  ...
  python scripts/score_stage_c3_root_routes_vllm.py \
    ...
    --gpu-memory-utilization 0.7 \
    --output "$lifecycle_root/root-scores.json" \
  >"$lifecycle_root/root-score.log" 2>&1

touch "$lifecycle_root/LIFECYCLE_GATE_PASSED"
find "$lifecycle_root" -type f ! -name sha256-manifest.txt \
